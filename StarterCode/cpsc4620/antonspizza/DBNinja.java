...
public final class DBNinja {
    ...
    public static void AddToInventory(Topping t, double toAdd) throws SQLException, IOException
    {
        ...
        String s = "alter table TOPPINGS\n";
        String s1 = "set INVENTORY_LEVEL = " + (t.getInv() + toAdd) + "\n";
        String s2 = "where " + "'" + t.getName() + "'";
        String statmentString = s + s1 + s2;
    ...
    */
    ...
    public static ArrayList<Order> getCurrentOrders() throws SQLException, IOException
    {
         ...
         */
        String query = "select * from ORDERS where STATUS LIKE '%Complete%';";
        PreparedStatement stmt = conn.prepareStatement(query);
        ResultSet rset = stmt.executeQuery();
        while(rset.next()){
            //String cust_query = "select "
            PreparedStatement stmt2 = conn.prepareStatement(cust_query);
            //ICustomer 
            //Order temp = new Order(rset.getInt(1), )
            //os.add()
        }
        conn.close();
        return os;
    ...
    public static double getBasePrice(String size, String crust) throws SQLException, IOException
    {
        ...
        String query = "select PRICE from BASE_PRICE where CRUST_TYPE = ? AND PIZZA_SIZE = ?;";
        PreparedStatement stmt = conn.prepareStatement(query);
	...
	*/
    ...
    private static Discount getDiscount(int ID)  throws SQLException, IOException
    {
        ...
        try{
            ResultSet rset = stmt.executeQuery();
            D = new Discount(rset.getString(NAME), rset.getDouble(PERCENT_OFF), rset.getDouble(DOLLAR_AMOUNT_OFF), rset.getInt(DISCOUNT_ID));
        }
        catch(SQLException e){
